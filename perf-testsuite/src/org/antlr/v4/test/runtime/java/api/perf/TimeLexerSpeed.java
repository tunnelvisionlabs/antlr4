...
public class TimeLexerSpeed { // don't call it Test else it'll run during "mvn test"
	...
	public List<String> streamFootprints = new ArrayList<String>();
...
//		System.out.println(VM.current().details());
	...
	public void load_new_utf8(String resourceName, int n) throws Exception {
		...
		for (int i = 0; i<n; i++) {
			InputStream is = streams[i];
			try {
				input[i] = CharStreams.fromStream(is, Charset.forName("UTF-8"), resourceName, streamLength);
			}
			finally {
				is.close();
			}
	...
	public void lex_legacy_java_utf8(int n, boolean clearLexerDFACache) throws Exception {
		...
		try {
			...
			try {
				...
				try {
					@SuppressWarnings("deprecation")
					CharStream input = new org.antlr.v4.runtime.ANTLRInputStream(br);
